from tkinter import *  # This imports all 'objects' and 'methods' from the file as if they were in this file... generally bad practice.   It DOES NOT import all module functions...
...
class MainWindow:
    def __init__(
        self, master
    ):  # you can use something other than 'master' if you'd like.
        self.screenChoice = 0
        self.main_leftFrame = Frame(
            master, borderwidth=1, background="light blue", width=20
        )
        self.main_leftFrame.grid(column=0, row=0, sticky=(E, W, N, S), rowspan=1)
        self.main_leftFrame.grid_columnconfigure(0, pad=20)
        self.main_leftFrame.grid_columnconfigure(
            1, pad=20
        )  # First number is the column - will imact both sides; second number is the spacing.
        ...
        self.pipelineDetail_Screen = Button(
            self.main_leftFrame,
            text="Pipeline Details",
            command=self.pipeline_Detail_Screen,
            width=15,
            height=4,
        )
        self.pipelineDetail_Screen.grid(column=0, row=1)

        self.iliDisplay_Screen = Button(
            self.main_leftFrame,
            text="Data",
            command=self.iliData_Display,
            width=15,
            height=4,
        )
        ...
        self.burst_Screen = Button(
            self.main_leftFrame,
            text="Burst Data",
            command=self.burstPressureDisplay,
            width=15,
            height=4,
        )
    ...
    def upload_Screen(self):
        ...
        self.frame1 = Frame(
            self.main_frame, borderwidth=1, relief="solid", padx=2, pady=2
        )
        self.frame1.place(relheight=1, relwidth=1)

        # DropDown list for ComboBox
        self.vendorDropDown = ttk.Combobox(
            self.frame1,
            value=["Test", "Onstream", "Encompass", "Rosen"],
            state="readonly",
        )
    ...
    def iliData_Display(self):
        ...
        self.treescrollY = Scrollbar(
            self.rightFrame, orient="vertical", command=self.tv1.yview
        )
        self.treescrollX = Scrollbar(
            self.rightFrame, orient="horizontal", command=self.tv1.xview
        )
        self.tv1.configure(
    ...
    def burstPressureDisplay(self):
        ...
        self.evaluationMethod = ttk.Combobox(
            self.rightFrame,
            value=[
                "Method 1: 0.67dL",
                "Method 2: 0.85dL",
                ...
                "Method 5: RSTRENG Effective Area",
            ],
            state="readonly",
        )
        ...
        self.displayValuesButton = ttk.Button(
            self.rightFrame, text="View Results", command=self.showValues
        )
        ...
        Label(
            ...
            font="Arial 14 bold",
            justify="center",
        ).grid(column=2, row=4, sticky="n,s,e,w")
        Label(
            self.rightFrame,
            text="Level 3",
            background="red",
            font="Arial 14 bold",
            justify="center",
        ).grid(column=3, row=4, sticky="n,s,e,w")
        Label(
            self.rightFrame,
            text="Level 4",
            background="green",
            font="Arial 14 bold",
            justify="center",
        ).grid(column=4, row=4, sticky="n,s,e,w")
        Label(
            self.rightFrame,
            text="Level 5",
            background="red",
            font="Arial 14 bold",
            justify="center",
        ).grid(column=5, row=4, sticky="n,s,e,w")
        ...
        Label(
            ...
            font="Arial 14 bold",
            justify="center",
        ).grid(column=0, row=5, sticky="n,s,e,w")
        Label(
            self.rightFrame,
            text="PF Value:",
            background="green",
            font="Arial 14 bold",
            justify="center",
        ).grid(column=0, row=6, sticky="n,s,e,w")
        Label(
            self.rightFrame,
            text="SF Value:",
            background="blue",
            font="Arial 14 bold",
            justify="center",
        ).grid(column=0, row=7, sticky="n,s,e,w")
    ...
    def screen2_Setup(self):
        ...
        self.detail_label_1 = ttk.Label(
            self.frame_left_upper,
            text="Pipeline Identification",
            background="white",
            font="Arial 14 bold",
            anchor="n",
            justify="center",
        ...
        self.client_label = ttk.Label(
            self.frame_left_upper, text="Client: ", background="white"
        )
        self.client_label.grid(column=0, row=1, sticky="e")
        self.license_label = ttk.Label(
            self.frame_left_upper, text="Pipeline License Number: ", background="white"
        )
        self.license_label.grid(column=0, row=2, sticky="e")
        self.inspectiondate_label = ttk.Label(
            self.frame_left_upper, text="Inspection Date: ", background="white"
        )
        self.inspectiondate_label.grid(column=0, row=3, sticky="e")
        self.detail_label_2 = ttk.Label(
            self.frame_left_lower,
            text="Pipeline Static Design Details",
            background="white",
            font="Arial 14 bold",
            width=30,
            anchor="n",
        ...
        self.nominalID_label = ttk.Label(
            self.frame_left_lower, text="Nominal ID: ", background="white"
        )
        self.nominalID_label.grid(column=0, row=5, sticky="e")
        self.nominalWT_label = ttk.Label(
            self.frame_left_lower,
            text="Nominal Wall Thickness (WT): ",
            background="white",
        )
        self.nominalWT_label.grid(column=0, row=6, sticky="e")
        self.corrosionAllowance_label = ttk.Label(
            self.frame_left_lower,
            text="Corrosion/Inspection Allowance: ",
            background="white",
        )
        self.corrosionAllowance_label.grid(column=0, row=7, sticky="e")
        self.SMYS_label = ttk.Label(
            self.frame_left_lower, text="SMYS: ", background="white"
        )
        self.SMYS_label.grid(column=0, row=8, sticky="e")
        self.designFactor_label = ttk.Label(
            self.frame_left_lower, text="Code Design Factor, F: ", background="white"
        )
        self.designFactor_label.grid(column=0, row=9, sticky="e")
        self.locationFactor_label = ttk.Label(
            self.frame_left_lower, text="Location Factor, L: ", background="white"
        )
        self.locationFactor_label.grid(column=0, row=10, sticky="e")
        self.jointFactor_label = ttk.Label(
            self.frame_left_lower, text="Joint Factor, L: ", background="white"
        )
        self.jointFactor_label.grid(column=0, row=11, sticky="e")
        self.temperatureFactor_label = ttk.Label(
            self.frame_left_lower, text="Temperature Factor, T: ", background="white"
        )
        self.temperatureFactor_label.grid(column=0, row=12, sticky="e")
        self.licensePressure_label = ttk.Label(
            self.frame_left_lower, text="License Pressure (kPa): ", background="white"
        )
        self.licensePressure_label.grid(column=0, row=13, sticky="e")
        self.operatingTemperature_label = ttk.Label(
            self.frame_left_lower,
            text="Normal Operating Temperature (oC): ",
            background="white",
        )
        self.operatingTemperature_label.grid(column=0, row=14, sticky="e")
        self.maximumPressure_label = ttk.Label(
            self.frame_left_lower,
            text="Maximum Possible Pressure (kPa): ",
            background="white",
        )
        ...
        self.client_label_input = ttk.Entry(
            self.frame_left_upper, textvariable=self.client_label_input
        )
        self.client_label_input.grid(column=1, row=1)
        self.license_label_input = StringVar()
        self.license_label_input = ttk.Entry(
            self.frame_left_upper, textvariable=self.client_label_input
        )
        self.license_label_input.grid(column=1, row=2)
        self.inspectiondate_label_input = StringVar()
        self.inspectiondate_label_input = ttk.Entry(
            self.frame_left_upper, textvariable=self.inspectiondate_label_input
        )
        self.inspectiondate_label_input.grid(column=1, row=3)
        self.nominalID_label_input = StringVar()
        self.nominalID_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.nominalID_label_input
        )
        self.nominalID_label_input.grid(column=1, row=5)
        self.nominalWT_label_input = StringVar()
        self.nominalWT_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.nominalWT_label_input
        )
        self.nominalWT_label_input.grid(column=1, row=6)
        self.corrosionAllowance_label_input = StringVar()
        self.corrosionAllowance_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.corrosionAllowance_label_input
        )
        self.corrosionAllowance_label_input.grid(column=1, row=7)
        self.SMYS_label_input = StringVar()
        self.SMYS_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.SMYS_label_input
        )
        self.SMYS_label_input.grid(column=1, row=8)
        self.designFactor_label_input = StringVar()
        self.designFactor_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.designFactor_label_input
        )
        self.designFactor_label_input.grid(column=1, row=9)
        self.locationFactor_label_input = StringVar()
        self.locationFactor_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.locationFactor_label_input
        )
        self.locationFactor_label_input.grid(column=1, row=10)
        self.jointFactor_label_input = StringVar()
        self.jointFactor_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.jointFactor_label_input
        )
        self.jointFactor_label_input.grid(column=1, row=11)
        self.temperatureFactor_label_input = StringVar()
        self.temperatureFactor_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.temperatureFactor_label_input
        )
        self.temperatureFactor_label_input.grid(column=1, row=12)
        self.licensePressure_label_input = StringVar()
        self.licensePressure_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.licensePressure_label_input
        )
        self.licensePressure_label_input.grid(column=1, row=13)
        self.operatingTemperature_label_input = StringVar()
        self.operatingTemperature_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.operatingTemperature_label_input
        )
        self.operatingTemperature_label_input.grid(column=1, row=14)
        self.maximumPressure_label_input = StringVar()
        self.maximumPressure_label_input = ttk.Entry(
            self.frame_left_lower, textvariable=self.maximumPressure_label_input
        )
        self.maximumPressure_label_input.grid(column=1, row=15)

        # Column 3 PL 100 Data (Labels)
        self.detail_label_3 = ttk.Label(
            self.frame_middle,
            text="PL 100 Data",
            background="white",
            font="Arial 14 bold",
        ...
        self.substance_label = ttk.Label(
            self.frame_middle, text="Substance: ", background="white"
        )
        ...
        self.from_label = ttk.Label(
            self.frame_middle, text="From: ", background="white"
        )
        self.from_label.grid(column=3, row=7, sticky="e")
        self.length_label = ttk.Label(
            self.frame_middle, text="Length (km): ", background="white"
        )
        self.length_label.grid(column=3, row=8, sticky="e")
        self.material_label = ttk.Label(
            self.frame_middle, text="Material: ", background="white"
        )
        self.material_label.grid(column=3, row=9, sticky="e")
        self.materialType_label = ttk.Label(
            self.frame_middle, text="Material Type: ", background="white"
        )
        self.materialType_label.grid(column=3, row=10, sticky="e")
        self.grade_label = ttk.Label(
            self.frame_middle, text="Grade: ", background="white"
        )
        self.grade_label.grid(column=3, row=11, sticky="e")
        self.stress_label = ttk.Label(
            self.frame_middle, text="Stress: ", background="white"
        )
        ...
        self.substance_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.substance_label_input
        )
        self.substance_label_input.grid(column=4, row=5)
        self.to_label_input = StringVar()
        self.to_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.to_label_input
        )
        self.to_label_input.grid(column=4, row=6)
        self.from_label_input = StringVar()
        self.from_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.from_label_input
        )
        self.from_label_input.grid(column=4, row=7)
        self.length_label_input = StringVar()
        self.length_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.length_label_input
        )
        self.length_label_input.grid(column=4, row=8)
        self.material_label_input = StringVar()
        self.material_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.material_label_input
        )
        self.material_label_input.grid(column=4, row=9)
        self.materialType_label_input = StringVar()
        self.materialType_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.materialType_label_input
        )
        self.materialType_label_input.grid(column=4, row=10)
        self.grade_label_input = StringVar()
        self.grade_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.grade_label_input
        )
        self.grade_label_input.grid(column=4, row=11)
        self.stress_label_input = StringVar()
        self.stress_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.stress_label_input
        )
        self.stress_label_input.grid(column=4, row=12)
        self.MOP_label_input = StringVar()
        self.MOP_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.MOP_label_input
        )
        self.MOP_label_input.grid(column=4, row=13)
        self.h2s_label_input = StringVar()
        self.h2s_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.h2s_label_input
        )
        self.h2s_label_input.grid(column=4, row=14)
        self.age_label_input = StringVar()
        self.age_label_input = ttk.Entry(
            self.frame_middle, textvariable=self.age_label_input
        )
        self.age_label_input.grid(column=4, row=15)

        # Column 5 Env Factors (Labels)
        self.detail_label_4 = ttk.Label(
            self.frame_right,
            text="Env Factors",
            background="white",
            font="Arial 14 bold",
        ...
        self.casedCrossing_label = ttk.Label(
            self.frame_right, text="Cased Crossings: ", background="white"
        )
        self.casedCrossing_label.grid(column=5, row=5, sticky="e")
        self.roadCrossing_label = ttk.Label(
            self.frame_right, text="Road Crossings: ", background="white"
        )
        self.roadCrossing_label.grid(column=5, row=6, sticky="e")
        self.railwayCrossing_label = ttk.Label(
            self.frame_right, text="Railway Crossings: ", background="white"
        )
        self.railwayCrossing_label.grid(column=5, row=7, sticky="e")
        self.stationCrossing_label = ttk.Label(
            self.frame_right, text="Station Crossings: ", background="white"
        )
        self.stationCrossing_label.grid(column=5, row=8, sticky="e")
        self.otherCrossing_label = ttk.Label(
            self.frame_right, text="Other Crossings: ", background="white"
        )
        self.otherCrossing_label.grid(column=5, row=9, sticky="e")
        self.substanceClass_label = ttk.Label(
            self.frame_right, text="Substance Class: ", background="white"
        )
        self.substanceClass_label.grid(column=5, row=10, sticky="e")
        self.pipelineClass_label = ttk.Label(
            self.frame_right, text="Pipeline Class: ", background="white"
        )
        self.pipelineClass_label.grid(column=5, row=11, sticky="e")
        self.seamType_label = ttk.Label(
            self.frame_right, text="Seam Type: ", background="white"
        )
        self.seamType_label.grid(column=5, row=12, sticky="e")

        # Column 6 User Input for Env Factors
        self.casedCrossing_label_input = ttk.Combobox(
            self.frame_right, value=["Yes", "No"], background="white", state="readonly"
        )
        self.casedCrossing_label_input.grid(column=6, row=5)
        self.roadCrossing_label_input = ttk.Combobox(
            self.frame_right, value=["Yes", "No"], background="white", state="readonly"
        )
        self.roadCrossing_label_input.grid(column=6, row=6)
        self.railwayCrossing_label_input = ttk.Combobox(
            self.frame_right, value=["Yes", "No"], background="white", state="readonly"
        )
        self.railwayCrossing_label_input.grid(column=6, row=7)
        self.stationCrossing_label_input = ttk.Combobox(
            self.frame_right, value=["Yes", "No"], background="white", state="readonly"
        )
        self.stationCrossing_label_input.grid(column=6, row=8)
        self.otherCrossing_label_input = ttk.Combobox(
            self.frame_right, value=["Yes", "No"], background="white", state="readonly"
        )
        self.otherCrossing_label_input.grid(column=6, row=9)
        self.substanceClass_label_input = ttk.Combobox(
            self.frame_right,
            value=["Gas", "Multiphase"],
            background="white",
            state="readonly",
        )
        self.substanceClass_label_input.grid(column=6, row=10)
        self.pipelineClass_label_input = ttk.Combobox(
            self.frame_right,
            state="readonly",
            background="white",
            value=[
                ...
                "prevelance of buildings intended for oiccupancy with 4 or more stories",
            ],
        )
        self.pipelineClass_label_input.grid(column=6, row=11)
        self.seamType_label_input = ttk.Combobox(
            self.frame_right,
            state="readonly",
            background="white",
            value=[
        ...
        if bool(self.pipelineDetails):
            ...
            try:
                ...
                pass
            try:
                self.license_label_input.insert(
                    0, self.pipelineDetails["License Number"]
                )
            except:
                pass
            try:
                self.inspectiondate_label_input.insert(
                    0, self.pipelineDetails["Inspection Date"]
                )
            except:
                pass
            try:
                ...
                pass
            try:
                self.nominalWT_label_input.insert(
                    0, self.pipelineDetails["Nominal Wall Thickness"]
                )
            except:
                pass
            try:
                self.corrosionAllowance_label_input.insert(
                    0, self.pipelineDetails["Corrosion Allowance"]
                )
            except:
                pass
            try:
                ...
                pass
            try:
                self.designFactor_label_input.insert(
                    0, self.pipelineDetails["Code Design Factor"]
                )
            except:
                pass
            try:
                self.locationFactor_label_input.insert(
                    0, self.pipelineDetails["Location Factor"]
                )
            except:
                pass
            try:
                self.jointFactor_label_input.insert(
                    0, self.pipelineDetails["Joint Factor"]
                )
            except:
                pass
            try:
                self.temperatureFactor_label_input.insert(
                    0, self.pipelineDetails["Temperature Factor"]
                )
            except:
                pass
            try:
                self.licensePressure_label_input.insert(
                    0, self.pipelineDetails["License Pressure"]
                )
            except:
                pass
            try:
                self.operatingTemperature_label_input.insert(
                    0, self.pipelineDetails["Normal Operating Temperature"]
                )
            except:
                pass
            try:
                self.maximumPressure_label_input.insert(
                    0, self.pipelineDetails["Max Operating Pressure"]
                )
            except:
                pass
            ...
            try:
                ...
                pass
            try:
                self.materialType_label_input.insert(
                    0, self.pipelineDetails["Material Type"]
                )
            except:
                pass
            try:
            ...
            try:
                self.casedCrossing_label_input.set(
                    self.pipelineDetails["Cased Crossing"]
                )
            except:
                pass
            try:
                ...
                pass
            try:
                self.railwayCrossing_label_input.set(
                    self.pipelineDetails["Railway Crossing"]
                )
            except:
                pass
            try:
                self.stationCrossing_label_input.set(
                    self.pipelineDetails["Stations Crossing"]
                )
            except:
                pass
            try:
                self.otherCrossing_label_input.set(
                    self.pipelineDetails["Other Crossings"]
                )
            except:
                pass
            try:
                self.substanceClass_label_input.set(
                    self.pipelineDetails["Substance Class"]
                )
            except:
                pass
            try:
                self.pipelineClass_label_input.set(
                    self.pipelineDetails["Pipeline Class"]
                )
            except:
                pass
            try:
        ...
        self.save_button = ttk.Button(
            self.frame_right, text="Save Data", command=self.save_pipelineDetail
        )
    ...
    def save_pipelineDetail(self):
        ...
        try:
            ...
            self.pipelineDetails[
                "Inspection Date"
            ] = self.inspectiondate_label_input.get()
            try:
                self.pipelineDetails["Nominal ID"] = float(
                    self.nominalID_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Nominal Wall Thickness"] = float(
                    self.nominalWT_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Corrosion Allowance"] = float(
                    self.corrosionAllowance_label_input.get()
                )
            except:
                pass
            try:
                ...
                pass
            try:
                self.pipelineDetails["Code Design Factor"] = float(
                    self.designFactor_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Location Factor"] = float(
                    self.locationFactor_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Joint Factor"] = float(
                    self.jointFactor_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Temperature Factor"] = float(
                    self.temperatureFactor_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["License Pressure"] = float(
                    self.licensePressure_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Normal Operating Temperature"] = float(
                    self.operatingTemperature_label_input.get()
                )
            except:
                pass
            try:
                self.pipelineDetails["Max Operating Pressure"] = float(
                    self.maximumPressure_label_input.get()
                )
            except:
                pass
            # Column 2
            ...
            self.pipelineDetails[
                "Cased Crossing"
            ] = self.casedCrossing_label_input.get()
            self.pipelineDetails["Road Crossing"] = self.roadCrossing_label_input.get()
            self.pipelineDetails[
                "Railway Crossing"
            ] = self.railwayCrossing_label_input.get()
            self.pipelineDetails[
                "Stations Crossing"
            ] = self.stationCrossing_label_input.get()
            self.pipelineDetails[
                "Other Crossings"
            ] = self.otherCrossing_label_input.get()
            self.pipelineDetails[
                "Substance Class"
            ] = self.substanceClass_label_input.get()
            self.pipelineDetails[
                "Pipeline Class"
            ] = self.pipelineClass_label_input.get()
    ...
    def showValues(self):
        ...
        try:
            self.mValue_1.insert(
                0, getExcel.newFeaturesList[self.featureiD.get()]["M Original"]
            )
        except:
            pass
        try:
            self.mValue_2.insert(
                0, getExcel.newFeaturesList[self.featureiD.get()]["M Modified"]
            )
        except:
            pass
        try:
            self.mValue_3.insert(
                0, getExcel.newFeaturesList[self.featureiD.get()]["M Modified"]
            )
        except:
            pass
        ...
        try:
            self.PF_Value1.insert(
                0,
                getExcel.newFeaturesList[self.featureiD.get()]["PF Level 1(Original)"],
            )
        except:
            pass
        try:
            self.PF_Value2.insert(
                0,
                getExcel.newFeaturesList[self.featureiD.get()]["PF Level 1(Modified)"],
            )
        except:
            pass
        try:
        ...
        try:
            self.SF_Value1.insert(
                0, getExcel.newFeaturesList[self.featureiD.get()]["SF Original"]
            )
        except:
            pass
        try:
            self.SF_Value2.insert(
                0, getExcel.newFeaturesList[self.featureiD.get()]["SF Modified"]
            )
        except:
            pass
        try:
    ...
    def display_Dictionary(self):
        ...
        for i in getExcel.newFeaturesList:  # Sort through Features (i = feature ID)
            ...
            for j in getExcel.newFeaturesList[
                i
            ]:  # Sort through feature specifics (J = specific information in row)
                # print(j)
    ...
    def safetyFactors(self):
        ...
        for i in self.sweetGasTable:
            if self.pipelineDetails["Substance Class"] == "Gas":
                self.pipelineDetails[i + " Safety Factor"] = self.sweetGasTable[i][
                    self.pipelineDetails["Class"]
                ]
            else:
                self.pipelineDetails[i + " Safety Factor"] = self.sourGasTable[i][
                    self.pipelineDetails["Class"]
                ]
        ...
        self.pipelineDetails["Joint Factor"] = self.weldType[
            self.pipelineDetails["Seam Type"]
        ]
        ...
        self.pipelineDetails["MaxDesignFactor"] = (
            self.pipelineDetails["Code Design Factor"]
            * self.pipelineDetails["Location Factor"]
            * self.pipelineDetails["Temperature Factor"]
        )
        self.pipelineDetails["SafetyFactor"] = (
            1 / self.pipelineDetails["MaxDesignFactor"]
        )
...
class missingValuesWindow:
    def __init__(self):
        ...
        Label(
            self.frame,
            text="You suck. Put in more info on page 2",
            height=10,
            width=15,
            wraplength=100,
        ).pack()
